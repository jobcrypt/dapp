...
const jobPostingPaymentCurrencySelect = ge(
  "job_posting_payment_currency_select"
);
...
async function createPosting() {
  ...
  jcFactoryFacadeContract.methods
    ...
    .then(function (response) {
      console.log(response);
      jobPostingCreateDisplay.innerHTML =
        "Draft Posting Created Txn :: " + response.blockHash;
      updateDraftListings();
    })
    .catch(function (err) {
      console.log(err);
...
async function saveToEVM(jobJSON, jobDescriptionHash, companySummaryHash) {
  ...
  postingEditorContract.methods
    ...
    .then(function (response) {
      console.log(response);
      jobPostingSaveDisplay.innerHTML =
        "Saved @> EVM :: " +
        response.blockHash +
        " :: IPFS COMPANY SUMMARY HASH :: " +
        companySummaryHash +
