...
function loadDashboard(dashboardAddress) {
  iEmployerDashboardContract = new web3.eth.Contract(
...
function buildEmployerDashboardTable(iEmployerDashboardContract) {
  iEmployerDashboardContract.methods
    .getPostings()
    .call({ from: account })
    .then(function (response) {
      console.log(response);
...
function buildPostingTableRow(postingAddress) {
  ...
  row.setAttribute("class", " grid grid-cols-7 text-sm mt-5 text-center");
...
function getExpiryDate(cell, iJCPostingContract) {
  iJCPostingContract.methods
    .getFeatureUINT("EXPIRY_DATE_FEATURE")
    .call({ from: account })
    .then(function (response) {
      console.log(response);
