...
function findDashboard() {
    console.log(jcFactoryFacadeContract);
    jcFactoryFacadeContract.methods.findDashboard("JOBSEEKER_DASHBOARD_TYPE").call({ from: account })
...
function addApplicationRow(postingAddress) {
    ...
    iJobPostingContract.methods.getFeatureUINT("APPLICANT_COUNT_FEATURE").call({
            from: account
        })
        .then(function(response) {
            console.log(response);
    ...
    iJobPostingContract.methods.getStatus().call({
            from: account
        })
        .then(function(response) {
            console.log(response);
