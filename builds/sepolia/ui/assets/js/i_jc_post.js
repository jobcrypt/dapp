...
async function createPosting() {
    ...
    jcFactoryFacadeContract.methods.createJobPosting(productAddress).send({ from: account })
        .then(function(response) {
            console.log(response);
            jobPostingCreateMsgDisplay1.innerHTML = "Draft Posting Created Txn :: " + response.blockHash;
            jobPostingCreateMsgDisplay2.innerHTML = "Draft Posting Created Txn :: " + response.blockHash;
            updateDraftListings();
        })
        .catch(function(err) {
            console.log(err);
...
function updatePaymentBox(productAddress, postingAddress) {
    ...
    productContract.methods.getPrice().call({ from: account })
        .then(function(response) {
            ...
            getErc20(productContract, jobPostingFee, price);
...
function setJobPostingFeeDisplay(erc20Address, jobPostingFee, price){
    iERC20 = getContract(ierc20MetadataAbi, erc20Address);
    iERC20.methods.decimals().call({from : account})
    .then(function(response){
        console.log(response);
        var decimals = response; 
        var factor = Number("1e"+decimals);
        jobPostingFee.innerHTML = price / factor; 
    })
    .catch(function(err) {
        console.log(err);
    })
...
function getErc20(productContract, jobPostingFee, price) {
    ...
    productContract.methods.getErc20().call({ from: account })
        .then(function(response) {
            ...
            setJobPostingFeeDisplay(erc20, jobPostingFee, price);
        })
        .catch(function(err) {
            console.log(err);
        })
